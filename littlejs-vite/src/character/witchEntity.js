...
/**  
 * WitchEntity — idle ghost with fade-out support  
 * ----------------------------------------------  
 * ✅ Directional idle loop animation  
 * ✅ Fade-out after scene events (window_scene_1)  
 * ✅ Optional debug controls (arrow keys + P)  
 */  
export class WitchEntity {  
  ...
  /** Begin smooth fade and mark as fading */  
  fadeOut(speed = 0.015) {  
    this.fading = true;  
  ...
  update(dt = 1 / 60) {  
    ...
    // fade logic  
    if (this.fading) {  
      this.alpha -= this.fadeSpeed * 60 * dt;  
      if (this.alpha <= 0) {  
        this.alpha = 0;  
        this.dead = true;  
      }  
