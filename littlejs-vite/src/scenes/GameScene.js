...
import { FogSystem } from '../environment/FogSystem.js'; // ✅ NEW IMPORT  
...
export class GameScene {  
  constructor(skipInit = false) {  
    ...
    this.fog = new FogSystem(); // ✅ NEW: Initialize fog system  
    this.events = null;  
  ...
  async onEnter() {  
    ...
    // ✅ NEW: Fog control helpers  
    window.toggleFog = () => {  
      const enabled = this.fog.toggle();  
      console.log(`[Fog] ${enabled ? 'Enabled' : 'Disabled'}`);  
    };  
    window.setFogDensity = (d) => {  
      this.fog.setDensity(d);  
      console.log(`[Fog] Density set to ${d}`);  
    };  
  ...
  update() {  
    ...
    this.fog.update(dt, this.player.pos); // ✅ NEW: Update fog system  
    this.player.update();  
  ...
  render() {  
    ...
    // ✅ NEW: Render fog mid-layer (volumetric particles)  
    ...
    // ✅ NEW: Render fog overlay (screen-space effects)  
